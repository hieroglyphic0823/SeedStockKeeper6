...
suspend fun runGeminiOcr(apiKey: String, context: Context, bitmap: Bitmap): String {
    // 初回実行時は少し待機
...
- 各 companionPlants オブジェクトには、"plant" に1つの植物名、"effects" にその植物の効果の配列を記載してください。
- 1つの植物に複数の効果がある場合は、同じ植物名で1つのオブジェクトにまとめて、effects配列に複数の効果を含めてください。
- 効果のフィールド（"effects"）は、以下の2桁のコードを**厳密に**使用してください。該当が明確でない場合は「99」を使用してください：
  ...
  calendar 配列について：
  - 各 calendar オブジェクトに、以下の5つのフィールドを含めてください：
    - "region": 地域名（寒地、寒冷地、温暖地、暖地、冷涼地、中間地、暖地など）
    - "sowing_start_date": 種まき開始日（"2025-03-01"形式）
    - "sowing_end_date": 種まき終了日（"2025-03-20"形式）
    - "harvest_start_date": 収穫開始日（"2025-06-01"形式）
    - "harvest_end_date": 収穫終了日（"2025-06-30"形式）
