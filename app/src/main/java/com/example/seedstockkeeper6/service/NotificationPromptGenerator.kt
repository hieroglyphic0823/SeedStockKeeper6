...
class NotificationPromptGenerator {
    ...
    fun generateMonthlyPrompt(
        ...
        return """
            ...
            - 農園主が「八兵衛」の場合:
              「八兵衛！$monthName 種についてよく心得ておけよ！」

            【現在の農園主】: $farmOwner

            【アドバイス】
            本文の最後に、以下の条件に従い、農園主のキャラクターと季節・天候に合った一言アドバイス（36文字以内）を1行添えてください：
            ...
            【署名ルール】
             最後に以下の条件に従って署名を加えてください：

              - 結び：この月の農作業を励ますメッセージ（36文字以内）を農園主のキャラクターに応じて生成してください
              - 署名：農園主に応じた署名を使用してください
            ...
            【出力形式】
            1) 人間向けの本文（文頭あいさつ〜3セクション＋一言アドバイス）を表示してください：
            🌱 今月まきどきの種:
            ⚠️ 終了間近:
            🌟 今月のおすすめ種:

            2) 本文の後に、以下形式でJSONを必ずコードブロックで出力してください（JSON以外の文字は含めない）：
            ```json
            {
