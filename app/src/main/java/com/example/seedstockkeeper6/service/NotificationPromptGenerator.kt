...
class NotificationPromptGenerator {
    ...
    fun generateMonthlyPrompt(
        ...
        return """
            ...
            【指示】
            1. 文頭に、農園主（「水戸黄門」「お銀」「八兵衛」のいずれか）に対応した一言あいさつを入れる
            2. 「今月まきどき」「終了間近」「おすすめ」の3セクションに分けてリスト表示する
            3. 各種については「今、なぜまくべきか」「注意点」など、簡潔に説明（36文字以内の1文）
            4. 全体はくどくなりすぎないように。文量を抑え、読みやすく親しみやすく。
            5. キャラクターに応じた言葉遣いにする（以下参照）
            6. 種名は必ず『』で囲んで表示する（例：『恋むすめ』『ニンジン』）
            
            【文頭フォーマット（必ず本文の先頭にそのまま出力）】
            - 農園主が「水戸黄門」の場合:
              「ご隠居様、$monthName となりました。さて、このたびは下記の作物について、進言申し上げまする。」
            - 農園主が「お銀」の場合:
              「お銀殿、$monthName となりましたな。ついては、下記の作物について進言申し上げまする。」
            - 農園主が「八兵衛」の場合:
              「八兵衛！$monthName にござるぞ！下記の種にてよく心得ておけよ！」
            
            以下の形式で回答してください：
            1) まず人間向けの本文を、以下の見出しで出力してください：
            🌱 今月まきどきの種:[ユーザーが持っている今月まける種]
            ⚠️ 終了間近:[ユーザーが持っている今月で終わる種]
            🌟 今月のおすすめ種:[参考情報から推奨される種]
            その後に、農園主のキャラクターに合った短い結びの一言（36文字以内）を1行で付けること。
            
            各セクションに該当する種がない場合は「該当なし」と表示してください。
            
            2) 本文の後に、機械可読なJSONを必ずコードブロックで追記してください（JSON以外の文字を含めないこと）。各配列要素は {"name":"種名", "desc":"短い説明"} のオブジェクトとします：
            ```json
            {
              "this_month": [{"name": "種名1", "desc": "説明1"}, {"name": "種名2", "desc": "説明2"}],
              "ending_soon": [{"name": "種名A", "desc": "説明A"}],
              "recommended": [{"name": "種名X", "desc": "説明X"}]
            }
            ```
