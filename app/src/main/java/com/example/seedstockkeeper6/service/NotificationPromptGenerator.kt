...
class NotificationPromptGenerator {
    ...
    fun generateMonthlyPrompt(
        ...
        return """
            ...
            3. 各種については「今、なぜまくべきか」「注意点」など、簡潔に説明（30文字以内で1文）
            ...
            - 農園主が「水戸黄門」の場合:
              「ご隠居様、このたびは$monthName　の下記の作物について、進言申し上げまする。」
            - 農園主が「お銀」の場合:
              「お銀殿、$monthName　の下記の作物について進言申し上げまする。」
            - 農園主が「八兵衛」の場合:
              「八兵衛！$monthName にござるぞ！下記の種にて心得ておけ！」
            ...
            2) 本文の後に、機械可読なJSONを必ずコードブロックで追記してください（JSON以外の文字を含めないこと）：
            ```json
            {
              "this_month": ["種名1", "種名2"],
              "ending_soon": ["種名A"],
